...
{
	if (ray->dir.x < 0)
		ray->side_dist.x = (ray->start.x - (float)ray->hit_pos.x) * ray->delta_dist.x;
	else
		ray->side_dist.x = ((float)ray->hit_pos.x + 1.0 - ray->start.x) * ray->delta_dist.x;
	if (ray->dir.y < 0)
		ray->side_dist.y = (ray->start.y - (float)ray->hit_pos.y) * ray->delta_dist.y;
	else
		ray->side_dist.y = ((float)ray->hit_pos.y + 1.0 - ray->start.y) * ray->delta_dist.y;
}
...
void			dist_round_block(t_ray *ray)
{
	...
	double c = ray->start.y - ray->start.x * m; 
...
t_bool			next_ray(t_ray *ray)
{
	...
	ray->hit = NULL;
	ray->fhit = NULL;
	if (ray->hit_pos.x < 0 || ray->hit_pos.x >= ray->world->size.x || ray->hit_pos.y < 0 || ray->hit_pos.y >= ray->world->size.y)
		return (FALSE);
	ray->hit = ray->world->data[ray->hit_pos.y][ray->hit_pos.x];
	ray->fhit = ray->hit;
	compute_face(ray);
